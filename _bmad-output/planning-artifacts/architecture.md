...
- No Jira, Azure DevOps, GitHub, GitLab, or other backlog integrations are required.
...
Full-stack real-time web application based on TypeScript, React, and Azure.
...
- Azure App Service supports Node.js apps, Node `24-lts`, Web sockets configuration, Always On, and session affinity.
...
- Can later move to Redis or Azure Web PubSub if scale or resilience needs increase.
...
**Option 2: Azure Static Web Apps + Azure Functions + Azure Web PubSub**

This option uses a static React frontend, Azure Functions for API endpoints, and Azure Web PubSub for live messaging.

Fit:

- Azure-native and serverless.
...
The selected foundation favors explicit state modeling over framework ceremony. For v1, a single Azure App Service Node app can serve the built React frontend and host the Socket.IO backend. The architecture should run as one instance initially so in-memory Session state remains coherent. If future requirements add durable sessions, reconnect recovery, multiple teams, or horizontal scale, the backend can introduce Redis or Azure Web PubSub without changing the frontend starter.
...
- Node.js runtime target should be aligned with current tool requirements; use Node `24-lts` for Azure App Service where available, with local development on a supported Node version.
...
- Vite provides fast local frontend development and production builds.
- Socket.IO backend should run locally alongside Vite during development.
- Azure App Service deployment should build the frontend, build the server, then start the Node server.
- App Service Web sockets should be enabled for Socket.IO.
- App Service Always On should be enabled when using a paid tier to reduce cold-start disruption.

**Note:** Project initialization using the Vite command should be the first implementation story. The same story should add the Node/Socket.IO server scaffold, shared types, test setup, and Azure App Service startup scripts.
...
- Run the MVP as a single Azure App Service Node instance.
...
- Use Application Insights and App Service log streaming for operational visibility.
...
- Azure Web PubSub is deferred until broadcast scale or managed real-time fan-out is needed.
...
- Use restrictive CORS: deployed Azure origin in production, localhost origins in development.
...
**Decision:** Deploy as one Node.js app on Azure App Service for Linux, single instance for MVP.

**Rationale:**
The selected architecture uses a long-running Node process that owns live in-memory Session state and Socket.IO connections. Azure App Service is the simplest Azure runtime for that shape. Static Web Apps plus Functions would add moving parts and still need a separate authoritative state service.

**Implementation Direction:**

- Deploy one Node.js application to Azure App Service for Linux.
...
- Use Node `24-lts` on App Service where available.
- The app must listen on `process.env.PORT`.
- Enable Web sockets in App Service.
- Enable HTTPS Only.
- Enable Always On for non-free production tiers.
- Run one App Service instance for MVP.
- Keep App Service session affinity enabled if more than one instance is ever tested before introducing shared state.
- Use GitHub Actions for CI/CD.
- Prefer OpenID Connect for GitHub Actions to Azure when available.
- Build and test in GitHub Actions before deploying compiled output.
- Use Azure App Settings for environment variables.
- Use Application Insights for server-side monitoring and App Service log streaming for operational troubleshooting.
- Do not add Redis, Azure Web PubSub, Azure SQL, Cosmos DB, or Azure Storage in MVP unless scope changes.

**Version Notes:**

- Microsoft App Service Node.js docs show `NODE|24-lts` and `WEBSITE_NODE_DEFAULT_VERSION="~24"` examples.
- Microsoft GitHub Actions deployment docs note compiled Node/TypeScript apps should build before deploying compiled output when using `azure/webapps-deploy@v3`.
...
11. Add Azure App Service production build/start scripts and CI/CD.
12. Add monitoring, logging, health endpoint, and deployment documentation.

**Cross-Component Dependencies:**

- In-memory state requires single-instance Azure App Service deployment.
...
- App Service deployment must serve both the static React build and Socket.IO from the same Node process.
...
No external product integrations exist in v1. Azure App Service, Application Insights, and GitHub Actions are deployment/operations integrations only.
...
The production build compiles the React app to `dist` and compiles the server to a Node-runnable output.

**Deployment Structure:**
Azure App Service starts the compiled Node server. The Node server serves `dist`, exposes `/health`, and hosts Socket.IO on the same origin.
...
All major choices work together: Vite React TypeScript provides the SPA, Express serves HTTP/static assets, Socket.IO handles live Session commands, Zod validates contracts, and Azure App Service hosts the long-running Node process required for in-memory room state.
...
The directory tree is specific enough for implementation agents to place client, server, shared, test, deployment, and documentation files consistently.
...
- Redis or Azure Web PubSub for multi-instance scale.
...
Initialize the Vite React TypeScript app, then add the Node/Express/Socket.IO server scaffold, shared contracts, Zod schemas, test setup, and Azure App Service build/start scripts.
