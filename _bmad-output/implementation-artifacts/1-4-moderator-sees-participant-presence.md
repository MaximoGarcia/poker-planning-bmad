...
- [x] [Review][Patch] Use a neutral accessible list label — `Joined participants` also contains disconnected entries rendered as `Away`, so the accessible name can misdescribe the list. [`src/features/session/ModeratorSessionView.tsx`:77]
- [x] [Review][Patch] Exercise two participant records in duplicate-name coverage — the component test filters out the moderator and only renders `Maxi (2)`, so it does not verify multiple participant labels. [`src/features/session/ModeratorSessionView.test.tsx`:88-103]
- [x] [Review][Patch] Make snapshot-preference coverage timestamp-aware — the test calls the live snapshot newer but does not provide a route timestamp or verify ordering behavior. [`src/features/session/ModeratorSessionView.test.tsx`:157-195]
- [x] [Review][Patch] Test sensitive-field filtering at the snapshot boundary — the privacy test casts an invalid snapshot containing token/card fields and therefore bypasses `SessionSnapshotSchema`; it does not verify the real transport/parser boundary. [`src/features/session/ModeratorSessionView.test.tsx`:124-155]
- [x] [Review][Patch] Add a unique-name live-join assertion — the E2E coverage only exercises the disambiguated duplicate-name path, leaving the normal participant display path indirect. [`tests/e2e/create-session.spec.ts`:41-54]
- [x] [Review][Patch] Assert the moderator URL remains unchanged during the live join — this would make the no-refresh behavior explicit instead of inferring it only from the rendered row. [`tests/e2e/create-session.spec.ts`:46-54]
- [x] [Review][Defer] Reject stale same-room snapshots — `ModeratorSessionView` prefers any same-room `latestSnapshot`, and `useSessionSocket` accepts snapshots without monotonic timestamp checks; this behavior predates Story 1.4 and is deferred to the snapshot-streaming work. [`src/features/session/ModeratorSessionView.tsx`:106-117; src/features/session/useSessionSocket.ts:100-105]
