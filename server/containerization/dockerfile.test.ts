...
expect(dockerfile).toContain('FROM node:20.19.0-alpine AS build')
expect(dockerfile).toContain('RUN npm ci')
expect(dockerfile).toContain('RUN npm run build')
expect(dockerfile).toContain('FROM node:20.19.0-alpine AS runtime')
})

it('runs as a non-root user and declares a healthcheck', () => {
const dockerfile = readFileSync(dockerfilePath, 'utf8')
